...
Deeper implementation detail that doesn't belong in the top-level README.
See also `infra/README.md` for the SAM/LocalStack track specifically.
...
3. **The spec's real AWS architecture, via SAM CLI + LocalStack**
   ...
   public** — bound to `localhost:4566`. See `infra/README.md`.
...
while `npm run dev:server` is running, restart it to pick up the change —
those packages resolve to their built `dist/` output (needed for the
Railway/production `node dist/index.js` start command to work at all), not
their live TypeScript source.
...
| 7 | Strands agent + Cedar | Both built. Cedar (`@cedar-policy/cedar-wasm`) gates the Society routes for real; Strands (`@strands-agents/sdk`, TypeScript preview) is an opt-in alternate ask-anytime agent behind `STRANDS_QA=true`, tool-calling real Gemini — falls back to the direct call on any error, since the SDK is experimental |
...
`packages/data/tariffs.ts` carries **sourced, real numbers**, not
...
- `packages/data/gridFacts.ts` — the national figures are the spec's own
  ...
  scenario there is what the "proof" demo slide replays.
...
- **WhatsApp → web chat, for the live demo.** `apps/server/src/whatsapp.ts`
  is a real, deployed Twilio WhatsApp webhook: it receives real inbound
  WhatsApp messages, runs the real `HouseholdService` conversation state
  machine, and was verified live end-to-end. What doesn't work on Twilio's
  **free trial** specifically is the *reply*: their trial account requires
  every outbound message (including replies to an inbound one) to use a
  pre-approved Content Template, and the Content API needed to even list
  those templates is itself locked behind a paid upgrade. So inbound
  WhatsApp messages update real household state, but reply text can't go
  back out without paying to leave Twilio's trial or rebuilding on Meta's
  Cloud API — both out of scope for this round. `apps/web/src/Chat.tsx` is
  the fully-working WhatsApp-style channel used for the demo. See "The
  Twilio WhatsApp webhook" below for the full detail.
...
Already deployed (see the live URLs in the README). To redeploy or
understand the setup: `.railway/railway.ts` is the source of truth
(Railway's "Infrastructure as Code" format — the older `railway.json`
config-as-code format is deprecated). It defines two services,
`bijli-server` and `bijli-web`, each with their `build`/`start` commands.
Root `package.json` scripts `build:server` / `build:web` are what those
commands call.
